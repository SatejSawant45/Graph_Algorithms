...
bool checkBipartiteness(map<ll,vector<ll>> &mp,int nodes,vector<ll> &color,ll currColor,ll node)
{
    queue<ll> que;
...
ll Union(ll x,ll y,vector<ll> &parent,vector<ll> &rank)
{
    ll xPr = find(x,parent);
    ll yPr = find(y,parent);

    if(xPr!=yPr)
    {
        if(rank[xPr]==rank[yPr])
        {
            parent[xPr] = yPr;
            rank[yPr]++;
        }
        else if(rank[xPr]>rank[yPr])
        {
            parent[yPr] = xPr;   
        }
        else
