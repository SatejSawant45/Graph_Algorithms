...
void DFS(int n,map<ll,vector<ll>> &mp,vector<ll> &visited,vector<ll> &parent,ll node)
{
    visited[node] = 1;
...
{   
    ...
    for(auto pr:mp)
    {
        ...
        if(!visited[node])
        {
            DFS(n,mp,visited,parent,node);
        }
    }

    reverse(topoVector.begin(),topoVector.end());
