...
void DepthFS(map<ll,vector<ll>> &mp,vector<ll> &degree,vector<ll> &visited,ll n,ll node)
{
    visited[node] = 1;
    vector<ll> v = mp[node];
    ...
    for(auto x:v)
    {
        if(!visited[x])
        {
            DepthFS(mp,degree,visited,n,x);
        }
    }
}
bool isConnected(map<ll,vector<ll>> &mp,vector<ll> &degree,ll n)
{
...
void eulerpath(map<ll,vector<ll>> &mp)
{
    ll n = mp.size();
    ...
    if(oddDeg>2)
    {
        cout<<"No euler path or euler circuit exists"<<endl;
    }
    else if(oddDeg==2)
    {
        cout<<"Only euler path exists"<<endl;
    }
    else if(oddDeg==0)
    {
        cout<<"Both euler path and euler circuit exists"<<endl;
    }
