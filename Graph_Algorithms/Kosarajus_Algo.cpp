...
stack<ll> topo;
void toposort(int n,map<ll,vector<ll>> &mp,vector<ll> &visited,ll node)
{
...
vector<ll> temp;
void DFS(map<ll,vector<ll>> &mp, vector<ll> &visited,ll node)
{
    visited[node] = 1;
    vector<ll> v = mp[node];
    temp.push_back(node);
...
{   
    ...
    // step-1 : generate a topo stack
    vector<ll> visited(n,0);
    for(ll i=0;i<n;i++)
    {
        if(!visited[i])
        {
            toposort(n,mp,visited,i);
        }
    }

    //step -2 : make a raverse graph
    ...
    for(auto pr:mp)
    {
        ...
        for(ll x:v)
        {
            mp1[x].push_back(node);
        }
    }

    debug(mp1); //reverse graph


    // step 3 : do dfs on nodes based on order given in stack;
    
    vector<vector<ll>> ssc;
