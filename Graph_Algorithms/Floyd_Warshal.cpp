...
vector<vector<ll>> floydwarshall(int nodes,vector<vector<ll>> &edges,ll src)
{
    ll sz = edges.size();


    vector<vector<ll>> res(nodes,vector<ll> (nodes,INF));
...
{   
    ...
    vector<vector<ll>> result = floydwarshall(nodes,edges,src);
