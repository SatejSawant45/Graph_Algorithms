...
vector<ll> dfsvector;
bool DFS(int n,map<ll,vector<ll>> &mp,vector<ll> &visited,vector<ll> &parent,ll node)
{
    ...
    for(auto ele:v)
    {
        ...
        {
            parent[ele] = node; 
            if(DFS(n,mp,visited,parent,ele)) return true;
        }

        if(visited[ele] && parent[node]!=ele)
        {
            return true;
        }
    }
...
{   
    ...
    for(auto pr:mp)
    {
        ...
        if(!visited[node] && DFS(n,mp,visited,parent,node))
        {
            cout<<"true"<<endl;
            return;
        }
